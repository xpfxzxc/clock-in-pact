...
- **动态流** - 支持小组动态页面，按时间倒序展示并支持分页；已接入小组创建、成员加入、目标创建/确认、目标修改/取消请求发起与投票、请求结果、打卡提交、审核、打卡确认/自动通过、目标状态变更、自动同意、新挑战者自动参与、结算完成、时长阶梯解锁等动态
- **结算与归档** - 目标结束日期到达后自动进入待结算状态；监督者确认结算后系统判定各挑战者达标情况；达标者自动解锁更长周期（时长阶梯）；结算结果公示含完成数值、百分比、达标状态与奖惩规则；目标归档后历史记录可查
